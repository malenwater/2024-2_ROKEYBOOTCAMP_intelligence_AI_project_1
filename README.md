...
| Property             | 없음                                                        |
| Misc                 | 샘플이 컨베이어 벨트 밖, 혹은 카메라 범위를 벗어날 경우, 고장 부위를 판별할 수 없기 때문에 샘플은 반드시 카메라 범위 안에 들어와 있어야 한다. 또한, 카메라 범위 안에는 한 개의 샘플만이 있어야 한다. 두 개의 샘플이 카메라 범위 안에 있을 경우, 둘 중 하나만 하자가 있어도 정상인 다른 샘플까지 모두 불량품으로 분류되기 때문.|
|Contact Info|이선우,김영수,한건희,최범석|
