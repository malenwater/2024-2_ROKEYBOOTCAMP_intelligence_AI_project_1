...
|[최범석](https://github.com/ausudu)|1. 주제를 명확하게 파악하지 못했다 2. 의견 주장을 확고하게 하지못했다 1. 라벨링과 모델생성 자체는 어렵지 않았으나 생성된 모델의 성능을 잘 파악하지 못했다. 결과가 나쁘지 않게 나왔으니 성능이 나쁜것이라 판단하고 라벨링을 다시하여 시간 소비가 많았던것 같다 . 프로젝트를 시작하기 전에 정확한 목표와 목표달성 기준을 정하고 들어가는것이 좋을 것같다. 2. 1,2주차에 자신의 부족함을 느끼고 프로젝트를 하는동안 자신의 생각과 주장이 맞는지에 대한 확신이 없어서 조원들에게 이야기를 꺼내지 못하고 말을 삼켰었다. 주장이 옭고 그름은 생각하지 않고 조금더 조원들과 생각을 교환했었더라면 좀더 좋은 결과가 나왔을 것같다|AI 모델 데이터 셋 구축, 데이터 전처리, AI 모델 성능 개선|
